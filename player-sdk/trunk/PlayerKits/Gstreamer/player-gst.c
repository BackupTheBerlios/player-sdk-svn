...
player_construct (Player *player, char **error)
{
	...
	if (!priv->play) {
		*error = g_strdup ("Failed to create GstPlay object");
	...
	if (!priv->source) {
		*error = g_strdup ("Failed to create the required gnomevfssrc GStreamer element");
	...
	if (!priv->sink) {
		*error = g_strdup ("Could not render default GStreamer audio output sink");
	...
	if (!priv->volume) {
		*error = g_strdup ("Could not find the volume element in the GstPlay pipeline");
